...
SEMANTIC_CACHE_VERSION = "segformer_ade20k_v2"
...
class SemanticMaskService:
    ...
    def _generate_cache(
        ...
        *,
        ...
        background_native = (
            cv2.bitwise_not(person_native)
            if background_found
            else np.zeros_like(person_native)
        )
        ...
        target_metadata["background"] = {
            ...
            "coverage": round(
                float(np.count_nonzero(background_native)) / float(background_native.size),
                6,
            ),
            ...
            "failure_reason": None
            if background_found
            else "person_not_found_for_background",
