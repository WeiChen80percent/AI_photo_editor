...
part 'editor_mobile_actions.dart';
...
class _EditorScreenState extends State<EditorScreen>
    with WidgetsBindingObserver, _EditorMobileActions {
  @override
  ...
  void initState() {
    ...
    _controller =
        widget.controller ??
        EditorController(
          api: ApiService(baseUrl: widget.workspace?.baseUrl),
    ...
    _controller.addListener(_rememberWorkspace);
    WidgetsBinding.instance.addObserver(this);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) unawaited(_bootstrapMobile());
    });
  }
  ...
  void dispose() {
    ...
    _controller.removeListener(_rememberWorkspace);
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Inactive can be the permission dialog itself; cancel only after the app
    // has really left the foreground, or an active recording is interrupted.
    if (state == AppLifecycleState.paused ||
        state == AppLifecycleState.hidden ||
        (state == AppLifecycleState.inactive &&
            _controller.isSpeechRecording)) {
      unawaited(_cancelMobileSpeech());
      _rememberWorkspace();
    }
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        return PopScope(
          canPop:
              !_controller.hasPendingDraft &&
              !_controller.isWorkspaceBusy &&
              !_mobileBusy,
          onPopInvokedWithResult: (didPop, result) {
            if (!didPop) unawaited(_handleMobileBack());
          },
          child: Scaffold(
            appBar: AppBar(
              toolbarHeight: 58,
              ...
              leading: _controller.sessionId == null && isNarrowAppBar
                  ...
                  : IconButton(
                      ...
                      onPressed: _mobileBusy || _controller.isWorkspaceBusy
                          ? null
                          : _confirmClearWorkspace,
              ...
              actions: [
                ...
                PopupMenuButton<String>(
                  key: const Key('mobile_actions'),
                  tooltip: l10n.mobileActions,
                  ...
                  itemBuilder: (_) => [
                    PopupMenuItem(
                      ...
                      enabled:
                          ...
                          !_mobileBusy &&
                          !_controller.isWorkspaceBusy &&
                          _controller.selectedEdit != null &&
                          !_controller.hasPendingDraft,
                      child: Text(l10n.mobileSave),
                    ),
                    PopupMenuItem(
                      value: 'restore',
                      enabled:
                          !_mobileBusy &&
                          !_controller.isWorkspaceBusy &&
                          widget.workspace?.sessionFor(_serverUrl) != null,
                      child: Text(l10n.mobileRestore),
                    ),
                    PopupMenuItem(
                      value: 'connection',
                      enabled:
                          ...
                          !_mobileBusy &&
                          !_controller.isWorkspaceBusy,
                      child: Text(l10n.mobileConnection),
                    ),
                  ],
                ),
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final shortLandscape =
                      isLandscape && constraints.maxHeight < 360;
                  final sideBySide =
                      isExpanded ||
                    ...
                    children: [
                      if (_mobileBusy)
                        const LinearProgressIndicator(
                          key: Key('mobile_progress'),
                        ),
                      if (_recoveryFailed)
                        Padding(
                          padding: const EdgeInsets.all(8),
                          child: Text(l10n.mobileRestoreRetry),
                        ),
  ...
  Future<void> _handleToolSelected(
    ...
    if (_mobileBusy ||
        (_controller.isWorkspaceBusy && _controller.activeTool != tool)) {
      return;
    }
  ...
  Future<void> _selectHistoryItem(EditHistoryItem item) async {
    if (_controller.isWorkspaceBusy || _mobileBusy) return;
  ...
  Future<void> _pickOriginal() async {
    if (_mobileBusy || _controller.isWorkspaceBusy) return;
    ...
    final bytes = await _pickImageBytes(role: 'original');
  ...
  Future<void> _pickReference() async {
    if (_mobileBusy || _controller.isWorkspaceBusy) return;
    final bytes = await _pickImageBytes(role: 'reference');
    if (bytes != null && mounted) {
      _controller.setReferenceImage(bytes);
    }
  }

  Future<Uint8List?> _pickImageBytes({required String role}) async {
    setState(() => _mobileBusy = true);
    try {
      final store = widget.workspace;
      if (store != null) {
        try {
          await store.beginPicker(_serverUrl, role);
        } catch (_) {
      ...
      final info = await MobileImageIO.read(file);
      if (mounted) {
        _mobileNotice(context.l10n.mobileImageSize(info.width, info.height));
      }
      return info.bytes;
    } catch (error) {
