...
class EditorController extends ChangeNotifier {
  ...
  /// Operations whose result must settle before replacing the workspace.
  bool get isWorkspaceBusy =>
      isRestoringSession ||
      isProcessing ||
      isPlanningCommand ||
      isRunningAutoModels ||
  ...
  Future<void> restoreSession(String id, {String? preferredEditId}) async {
    ...
    try {
      ...
      if (session.edits.isEmpty) {
        throw const ApiException(
          statusCode: 404,
          code: 'session_empty',
          message: 'The saved session has no completed versions.',
        );
      }
      ...
      originalImageUrl = history.first.originalUrl;
      selectedEdit = preferredEditId == originalParentSentinel
          ? null
          : _findEdit(preferredEditId ?? '') ?? history.last;
      selectedEditId = selectedEdit?.editId ?? originalParentSentinel;
      ...
      if (generation == _workspaceGeneration && !_disposed) {
  ...
  Future<bool> _submitEdit({
    ...
    try {
      ...
      if (_disposed || generation != _workspaceGeneration) return false;
      _applyCommittedItem(item);
      await refreshHistory(preferredEditId: item.editId, quiet: true);
  ...
  Future<bool> refreshHistory({
    ...
    try {
      final session = await _api.fetchSession(id);
      if (_disposed || sessionId != id) return false;
      history = session.edits;
      final targetId =
          preferredEditId ??
          selectedEditId ??
  ...
  void _applyCommittedItem(EditHistoryItem item) {
    ...
    manualPreview = null;
    _ensureComparisonState();
    // Persist the acknowledged commit before optional history/schema refreshes.
    _notify();
  }
